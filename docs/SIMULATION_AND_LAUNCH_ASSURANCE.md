...
The complete deterministic inventory is available as a human-readable Excel workbook at [`apps/web/public/simulation/eventos-synthetic-catalogue.xlsx`](../apps/web/public/simulation/eventos-synthetic-catalogue.xlsx). It includes a start guide, all 150 fictional businesses, all 1,350 named listings with descriptions, a visual gallery of the generated catalogue images and the first multi-supplier event scenario. The machine-readable source remains available at [`apps/web/public/simulation/eventos-synthetic-catalogue.csv`](../apps/web/public/simulation/eventos-synthetic-catalogue.csv). Regenerate the CSV with `pnpm --filter api simulation:export-catalogue` whenever the fixture definition changes, then refresh the Excel delivery workbook from that source.
