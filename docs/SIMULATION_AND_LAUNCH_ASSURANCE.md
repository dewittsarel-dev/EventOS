...
The deterministic catalogue contains 1,350 synthetic offerings across the 150 fictional businesses. Original AI-generated fixture families represent supplier stock, planner services, venues and technical specialists. Every listing is labelled synthetic and states that it is unavailable for real purchase.

The isolated browser catalogue is available at `/marketplace/simulation`. It reads a public-safe JSON projection containing only customer-facing simulator fields; internal cost prices are deliberately excluded. The normal Marketplace remains connected only to supplier-published records.

The visual catalogue includes original product-family photography for chairs, banquet/cocktail tables, floral centrepieces, modular backdrops, place settings, linen, ottomans, plinths, decorative lighting, staging, tents, catering equipment and fictional venues. Detailed listings include individual cutlery, underplates, crockery, glassware, napkin rings, table runners, napkins, candles, lounge pieces and compact display items. These images were generated specifically for EventOS, contain no real supplier identity or copied catalogue content, and are mapped to realistic synthetic listings for Marketplace presentation, search and AI mood-board testing.

The first visual multi-supplier scenario is exported as `visual-multi-supplier-event.json`. It assembles a 100-guest synthetic mountain wedding from five fictional businesses, creates a four-zone AI mood-board draft, preserves explicit operator approval, and exercises stock substitution, weather fallback and budget-change responses without creating production commitments.

The complete deterministic inventory is available locally to operators as a human-readable Excel workbook at [`apps/web/public/simulation/eventos-synthetic-catalogue.xlsx`](../apps/web/public/simulation/eventos-synthetic-catalogue.xlsx). It includes a start guide, all 150 fictional businesses, all 1,350 named listings with descriptions, a visual gallery of the generated catalogue images and the first multi-supplier event scenario. The machine-readable source remains at [`apps/web/public/simulation/eventos-synthetic-catalogue.csv`](../apps/web/public/simulation/eventos-synthetic-catalogue.csv). Browser access to both operator files is blocked because they include internal simulator pricing; Marketplace uses only the public-safe JSON projection. Regenerate the CSV with `pnpm --filter api simulation:export-catalogue` whenever the fixture definition changes, then refresh the Excel delivery workbook from that source.

Operator-owned supplier photographs are kept outside version control under `Supplier Photos/`. Running `pnpm --filter api simulation:audit-photos` creates a non-destructive audit with filename-based catalogue matches, exact-content duplicate detection, manual-review candidates and missing-image coverage. No photograph is copied to a public location without an explicit rights and publication review.
