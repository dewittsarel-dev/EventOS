...
export function MarketplaceHeader({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-stone-200/80 bg-[#fffdf9]/95 px-4 backdrop-blur-xl md:px-10">
      <div className={`mx-auto flex max-w-7xl items-center justify-between gap-3 ${compact ? 'h-16' : 'h-[4.5rem]'}`}>
        <Link href="/marketplace" className="flex min-w-0 items-center gap-3" aria-label="EventOS Marketplace home">
        ...
        <nav aria-label="Marketplace navigation" className="flex shrink-0 items-center gap-2">
          <Link href="/marketplace" className="hidden rounded-full px-3 py-2 text-sm font-medium text-stone-600 hover:bg-stone-100 hover:text-stone-950 md:block">Browse</Link>
