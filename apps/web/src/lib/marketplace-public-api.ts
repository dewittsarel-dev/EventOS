...
export const updateCustomerEventConceptSelection = (token: string, conceptId: string, resourceId: string, input: { discoveryPath?: MarketplaceEventConcept['lastDiscoveryPath']; quantity?: number; notes?: string }) => customerRequest<MarketplaceEventConcept>(token, `/event-concepts/${conceptId}/selections/${resourceId}`, { method: 'PATCH', body: JSON.stringify(input) });
export const replaceCustomerEventConceptSelection = (token: string, conceptId: string, resourceId: string, input: { replacementResourceId: string; discoveryPath: MarketplaceEventConcept['lastDiscoveryPath']; quantity?: number; notes?: string }) => customerRequest<MarketplaceEventConcept>(token, `/event-concepts/${conceptId}/selections/${resourceId}/replace`, { method: 'POST', body: JSON.stringify(input) });
