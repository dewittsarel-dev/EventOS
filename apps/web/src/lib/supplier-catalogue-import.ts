...
const categoryRules: Array<[SupplierProductCategory, RegExp]> = [
  ...
  [
    "Catering",
    /catering|glass|plate|fork|knife|spoon|crockery|cutlery|chafing|bar/i,
  ],
  ["Printing", /print|sign|menu|invitation|stationery|banner/i],
  [
    "Decor",
    /decor|chair|table|linen|runner|flower|floral|vase|plinth|ottoman|sofa|couch|backdrop|arch/i,
  ],
  [
    "Service",
    /planner|photograph|videograph|dj|entertain|security|staff|coordination/i,
  ],
...
function field(
  ...
  name: keyof typeof headerAliases,
) {
...
function categoryFor(text: string, supplied: string): SupplierProductCategory {
  const direct = categoryRules.find(
    ([category]) => normalize(category) === normalize(supplied),
  );
  if (direct) return direct[0];
  return (
    categoryRules.find(([, pattern]) =>
      pattern.test(`${supplied} ${text}`),
    )?.[0] ?? "Other"
  );
}
...
export function extractCsvCatalogue(
  text: string,
  sourceName: string,
): CatalogueImportCandidate[] {
  ...
  return rows.slice(1).map((values, index) => {
    ...
    if (costPrice === undefined)
      issues.push("Cost price requires confirmation.");
    ...
    return {
      ...
      imageUrls: field(record, "imageUrls")
        .split(/[;|]/)
        .map((item) => item.trim())
        .filter(Boolean),
...
function candidateFromTextBlock(
  block: string,
  sourceName: string,
  sourceRow: number,
): CatalogueImportCandidate | null {
  const lines = block
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const productName =
    lines.find(
      (line) =>
        /[a-zA-Z]{3}/.test(line) && !/^(page|catalogue|price list)/i.test(line),
    ) ?? "";
  if (!productName) return null;

  const priceLine =
    lines.find((line) => /(?:R|ZAR|\$)\s*[\d.,]+/i.test(line)) ?? "";
  const sellingPrice = numberValue(priceLine);
  const dimensions =
    lines.find((line) => /\b\d+(?:[.,]\d+)?\s*(?:mm|cm|m)\b/i.test(line)) ?? "";
  const description = lines
    .filter((line) => line !== productName && line !== priceLine)
    .slice(0, 4)
    .join(" ");
  const searchTerms = proposedTerms(`${productName} ${description}`, []);
  ...
  return {
    ...
    issues: [
      ...
      ...(sellingPrice === undefined
        ? ["Selling price was not detected."]
        : []),
...
export function extractTextCatalogue(
  text: string,
  sourceName: string,
): CatalogueImportCandidate[] {
  ...
  return blocks
    ...
    .filter(
      (candidate): candidate is CatalogueImportCandidate => candidate !== null,
    );
}

export function candidateToPayload(
  candidate: CatalogueImportCandidate,
